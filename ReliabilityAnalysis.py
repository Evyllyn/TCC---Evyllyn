...
class DadosMaterial:
        ...
        for i, (x, y) in enumerate(zip(self.X_cont, self.Y_cont)):
            ...
            if condition_x == 0 and condition_y == 0:
                plt.plot(x, y, 'ks', markersize=10,
                         label="Engaste" if 'Engaste' not in plt.gca().get_legend_handles_labels()[1] else "")
            elif condition_x == 0 and condition_y == 1:
                plt.plot(x, y, 'ms', markersize=10,
                         label="Apoio 1º Gênero" if 'Apoio 1º Gênero' not in plt.gca().get_legend_handles_labels()[
                             1] else "")
            elif condition_x == 1 and condition_y == 0:
                plt.plot(x, y, 'gs', markersize=10,
                         label="Apoio 2º Gênero" if 'Apoio 2º Gênero' not in plt.gca().get_legend_handles_labels()[
                             1] else "")

            # Forças aplicadas
            if force_x != 0:
                plt.arrow(x, y, force_x * 0.1, 0, head_width=0.05, head_length=0.1, fc='green', ec='green',
                          label="Força X" if 'Força X' not in plt.gca().get_legend_handles_labels()[1] else "")
            if force_y != 0:
                plt.arrow(x, y, 0, force_y * 0.1, head_width=0.05, head_length=0.1, fc='blue', ec='blue',
                          label="Força Y" if 'Força Y' not in plt.gca().get_legend_handles_labels()[1] else "")
    ...
    def Saida_dados(self):
        arquivoS = open("Saida_Exemplo_1_32.txt", "wt")
        ...
        for i in range(1, self.Num_P_int + 1):
            arquivoS.write(
                "%4d %10.4f %10.4f %14.12f %10.12f %15.4f %10.4f %10.4f %12.4f %10.4f %6.1f %6.1f\n"
                % (
                    i - 1,
                    self.X_int[i - 1],
                    self.Y_int[i - 1],
...
class MonteCarloReliability:
    def __init__(self, num_simulacoes, media_E, desvio_padrao_E, media_v, desvio_padrao_v, caminho_arquivo):
        self.num_simulacoes = num_simulacoes
        self.media_E = media_E
        self.desvio_padrao_E = desvio_padrao_E
        self.media_v = media_v
        self.desvio_padrao_v = desvio_padrao_v
        self.resultados = []
        self.caminho_arquivo = caminho_arquivo

    def gerar_amostras(self):
        self.amostras_E = np.random.normal(self.media_E, self.desvio_padrao_E, self.num_simulacoes)
        self.amostras_v = np.random.normal(self.media_v, self.desvio_padrao_v, self.num_simulacoes)

    def simular(self):
        self.resultados = []
        for E, v in zip(self.amostras_E, self.amostras_v):
            resultado = self.calcular_resultado(E, v)
            self.resultados.append(resultado)

    def calcular_resultado(self, E, v):
        dados_material_instancia = DadosMaterial(caminho_arquivo=self.caminho_arquivo)
        dados_material_instancia.ler_dados()
        dados_material_instancia.E = E
        dados_material_instancia.v = v
        dados_material_instancia.principalMEC2DSingle2()

        # Verificação corrigida para calcular tensao_max
        if hasattr(dados_material_instancia, 'Tens_Int') and dados_material_instancia.Tens_Int.size > 0:
            tensao_max = np.max(dados_material_instancia.Tens_Int)
        else:
            tensao_max = 0
    ...
    def plot_resultados(self):
        ...
        plt.subplot(1, 2, 1)
        plt.hist(self.amostras_E, bins=30, alpha=0.7, label='Módulo de Elasticidade')
        plt.xlabel('Módulo de Elasticidade')
        plt.ylabel('Frequência')
        plt.legend()
        plt.subplot(1, 2, 2)
        plt.hist(self.amostras_v, bins=30, alpha=0.7, label='Coeficiente de Poisson')
        plt.xlabel('Coeficiente de Poisson')
        plt.ylabel('Frequência')
        plt.legend()
        plt.show()
        plt.figure()
        plt.scatter(self.amostras_E, self.resultados, alpha=0.6, label='Resultados vs Módulo de Elasticidade')
        plt.xlabel('Módulo de Elasticidade')
        plt.ylabel('Resultado')
        plt.legend()
        plt.show()
        plt.figure()
        plt.scatter(self.amostras_v, self.resultados, alpha=0.6, label='Resultados vs Coeficiente de Poisson')
        plt.xlabel('Coeficiente de Poisson')
        plt.ylabel('Resultado')
        plt.legend()
        plt.show()
    ...
    def analisar_resultados(self):
        media_resultados = np.mean(self.resultados)
        desvio_padrao_resultados = np.std(self.resultados)
        print(f'Média dos Resultados: {media_resultados}')
        print(f'Desvio Padrão dos Resultados: {desvio_padrao_resultados}')


# Exemplo de uso
if __name__ == "__main__":
    caminho_do_arquivo = "Exemplo_1_16.txt"
    dados_material_instancia = DadosMaterial(caminho_arquivo=caminho_do_arquivo)
    dados_material_instancia.ler_dados()
    dados_material_instancia.principalMEC2DSingle2()

    num_simulacoes = 1000
    media_E = 210e9
    desvio_padrao_E = 10e9
    media_v = 0.3
    desvio_padrao_v = 0.05

    mc_reliability = MonteCarloReliability(num_simulacoes, media_E, desvio_padrao_E, media_v, desvio_padrao_v,
                                           caminho_do_arquivo)
    mc_reliability.gerar_amostras()
    mc_reliability.simular()
    mc_reliability.plot_resultados()
    mc_reliability.analisar_resultados()
