...
class DadosMaterial:
    # ALTERAÇÃO GABRIEL
    def __init__(self, caminho_arquivo, force_node ):
    ...
    def Calcula_D(self):
        ...
        for i in range(1, self.Num_elem + 1):
            self.Desloc=(0.022-self.DeslocYMax)
        
    def Saida_dados(self):
        arquivoS = open("SAida_viga.txt", "wt")
...
F_med = 27  # Força aplicada na chapa.
MONTE_CARLO_REP= 10
monte_carlo_F = norm.rvs(size=MONTE_CARLO_REP,loc=1.2*F_med,scale=1.2*.1*F_med)
Pf = 0
passou = True
...
for i in range(MONTE_CARLO_REP):
    dados_material_instancia = DadosMaterial(caminho_arquivo=caminho_do_arquivo, force_node=monte_carlo_F[i])
    tensao_vm = dados_material_instancia.principalMEC2DSingle2()
    print(f'TENSAO_VM: {tensao_vm}, Força: {monte_carlo_F[i]} ')
...
limite_max = max(max(monte_carlo_T_E), max(monte_carlo_F))
