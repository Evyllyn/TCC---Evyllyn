...
class DadosMaterial:
    ...
    def comprimento(self, j, k):
        L = np.sqrt((self.X_cont[k] - self.X_cont[j]) ** 2 + (self.Y_cont[k] - self.Y_cont[j]) ** 2)
        return L

    def plot_malha(self):
        """
        Visualiza a malha discretizada com os nós, elementos, condições de contorno e forças aplicadas.
        """

        plt.figure(figsize=(10, 10))

        # Verificar se os dados foram carregados corretamente
        if not hasattr(self, 'X_cont') or not hasattr(self, 'Y_cont') or not hasattr(self, 'conect'):
            print("Dados de malha não encontrados. Certifique-se de que os dados foram lidos corretamente.")
            return

        # Plotar os nós de contorno
        plt.scatter(self.X_cont, self.Y_cont, c='blue', label='Nós Contorno', zorder=3)
        if hasattr(self, 'X_int') and hasattr(self, 'Y_int'):
            plt.scatter(self.X_int, self.Y_int, c='red', label='Nós Internos', zorder=3)

        # Conectar os nós para formar os elementos
        for i, elem in enumerate(self.conect):
            try:
                x_coords = [self.X_cont[elem[0]], self.X_cont[elem[1]]]
                y_coords = [self.Y_cont[elem[0]], self.Y_cont[elem[1]]]
                plt.plot(x_coords, y_coords, 'k-', zorder=1, label="Elemento" if i == 0 else "")
            except IndexError:
                print(f"Erro ao acessar o nó do elemento {i}. Verifique a conectividade.")

        # Adicionar condições de contorno
        if hasattr(self, 'codigo') and hasattr(self, 'valor'):
            for i, (x, y) in enumerate(zip(self.X_cont, self.Y_cont)):
                if self.codigo[2 * i] == 0 and self.codigo[2 * i + 1] == 0:  # Engaste
                    plt.plot(x, y, 'ks', markersize=10, label="Engaste" if i == 0 else "")
                else:
                    plt.plot(x, y, 'gs', markersize=10, label="Apoio Simples" if i == 0 else "")

        # Adicionar as forças aplicadas
        if hasattr(self, 'valor'):
            for i, (x, y) in enumerate(zip(self.X_cont, self.Y_cont)):
                fx = self.valor[2 * i]  # Força na direção x
                fy = self.valor[2 * i + 1]  # Força na direção y
                if fx != 0 or fy != 0:  # Verificar se há força no nó
                    plt.arrow(
                        x, y, fx / 10, fy / 10,  # Escalar forças para melhorar visualização
                        head_width=0.5, head_length=0.5, fc='magenta', ec='magenta',
                        label="Força Aplicada" if i == 0 else ""
                    )

        # Rótulos dos nós
        for i, (x, y) in enumerate(zip(self.X_cont, self.Y_cont)):
            plt.text(x, y, f'{i}', fontsize=8, ha='right', color='blue')

        # Configurações do gráfico
        plt.title("Visualização da Malha Discretizada com Forças Aplicadas")
        plt.xlabel("X")
        plt.ylabel("Y")
        plt.axis('equal')
        plt.grid(True)
    ...
    def Saida_dados(self):
        arquivoS = open("SAida_exemplo.txt", "wt")
        ...
        for i in range(1, self.Num_P_cont + 1):
            print(f'Desloc - y: {self.FD[2 * i - 1]} ')
            arquivoS.write(
